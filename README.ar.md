...
## 🚨 حدود صلاحيات الـ Root ومتغيرات البيئة

عند تشغيل الـ VPN، تعد صلاحيات المستخدم الخارق (root) ضرورية ولا غنى عنها:

1.  **إعادة تعيين متغير `$PATH` باستخدام `sudo`**: افتراضيًا، يحد الأمر `sudo` من مسارات تنفيذ الأوامر (توجيه `secure_path` في `/etc/sudoers`). لتجنب عدم عثور سكربت التشغيل على الأدوات المساعدة المحلية الخاصة بك (`bin/redsocks`، `bin/dns2socks`)، قم دائمًا بتشغيل البيئة من مجلد المشروع الرئيسي.
2.  **صلاحيات الإدارة المطلوبة (Root)**: تتطلب تهيئة تدفقات التوجيه الشاملة، وفتح الواصفات منخفضة المستوى، وإعداد قواعد iptables NAT صلاحيات المستخدم الخارق (Root). على نظام أندرويد (Termux)، يجب أن يكون الجهاز مكسور الحماية (Rooted) بشكل قطعي.
