...
* **New `vaydns` connection mode**: `src/vaydns.py` manages a stack of N `vaydns-client` backends (`-tcp/-pubkey-file/-domain/-listen`, count+auto ports from `OMNI_VAYDNS_BASE_PORT` `2222`, `+1` until free), each fronted by a persistent OpenSSH `ssh -D` forwarder (one handshake, multiplexed streams — sing-box's `ssh` outbound dials per connection and parallel handshakes collapse over DNS) on auto ports (`OMNI_VAYDNS_SOCKS_BASE` `2240`), sing-box keeps the frontend (SOCKS/HTTP or TUN, DoH hijack, UDP fast-reject) with one `socks` outbound per forwarder behind round-robin `urltest` (N==1 skips the group, routes direct); SSH creds are the shared ssh flow (`sshpass -e`, never cmdline; publickey via shared key lookup); `vaydns://user:pass@domain?tcp&pubkey(pubkey-file)&instances=1..16#Remark` links import (pubkey string materialized to `cfgs/vaydns/`) and export; mode-aware status/Edit; launcher brings up clients→waits banners→forwarders→waits SOCKS→generates (fail-closed, trap reaps all, per-instance ports files); 77 tests green, stack e2e'd with stubs (`check`-clean, zero leftovers).
