...
## Unreleased — VayDNS transport

* **New `vaydns` connection mode**: `src/vaydns.py` manages N `vaydns-client` backends (`-tcp/-pubkey-file/-domain/-listen`, e.g. `./vaydns-client -tcp 8.8.8.8:53 -pubkey-file server.pub -domain vay.krel.qzz.io -listen 127.0.0.1:2222`) — count+auto ports from `OMNI_VAYDNS_BASE_PORT` (`2222`, `+1` until free), detached spawn with `/tmp` pidfiles, group-kill stop — plus a sing-box config with one `ssh` outbound per `127.0.0.1` backend behind a single balancing group (`route.final` + DoH detour ride it); `type: loadbalance` is a brand-new upstream PR (post-1.14.1, strategies hyphenated `round-robin`) decoded by no released binary — the lx fork instead extends `urltest` with `mode: round_robin` + `balancer`, which is what we emit (per-connection rotation over all backends, `pool` = instance count, stickiness off), verified with real `sing-box-lx check`; SSH creds are the shared ssh flow (link userinfo → `[ssh] username/password`, host/port untouched); `vaydns://user:pass@domain?tcp&pubkey-file&instances=1..16#Remark` links import (pubkey copied to `cfgs/vaydns/`, fail-closed hints when binary/pubkey missing) and export via Import menu option 2 / new Share option 4; mode-aware status, Edit rows (domain/transport/pubkey/instances + SSH creds), and Connection Mode entry; `vpn/singbox_proxification` spawns-then-generates with per-instance ports file and a trap-guarded non-exec run tail (backends die with sing-box), `runvpn.sh` routes vaydns straight to the launcher like v2ray with scoped/global client kills; 15 new tests (link round-trip, stub spawn/stop, real `sing-box-lx check`) — 66 total green; validated end to end with a stub binary (3 backends, exact CLI, `check`-clean config, zero leftover pidfiles).
